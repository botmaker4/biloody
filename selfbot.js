  ...
  if (command === "stealpfp"){
    ...
    if (command === "purge"){
         ...
         msg.channel.fetchMessages({limit:100}).then(async messages => {
             ...
             await msg.channel.send("Successfully purged " + ` \`${args[0]}/${args[0]}\`` + " messages!").then(message => message.delete(1500));
         })
          };
...
if (command === "translist") {
    ...
    msg.channel.send(embed);
     ...
     if (command === "help"){
        ...
        if (args.length < 1){
         let embed = new Discord.RichEmbed()
                .setDescription("`Prefix:` `x` `made by:` `P1` `Credits: Syrus`")
                .setAuthor("𝕀ℂ𝔼 𝕊𝔼𝕃𝔽𝔹𝕆𝕋")
                .setColor("#000000")
                .setThumbnail('https://media1.tenor.com/images/2c3a9bddb6fff6667c812c6ed57ad4d0/tenor.gif?itemid=16297708')
                .setTimestamp()
                .addField(`ℂ𝕆𝕄𝕄𝔸ℕ𝔻𝕊`, '`HELP TEXT` `HELP SPAMMER` `HELP FUN` `HELP FUN2` `HELP INFO` `HELP NUKE` `HELP SETUP`')
               .addField(`ℝ𝔼𝕊ℙ𝔼ℂ𝕋 𝕌𝕊 𝔸ℕ𝔻 𝕀𝕊 𝔸𝕃𝕃 𝔽𝕀ℕ𝔼`, '`✅NITRO SNIPER`  `✅MESSAGE LOGGER`')
                .setFooter(msg.author.tag, msg.author.displayAvatarURL);
                msg.channel.send(embed).then(message => message.delete(240000));
    
               msg.channel.send(embed).then(message => message.delete(240000)); 
        }else
        if (haste.toLowerCase() === "setup") {
            let gay = new Discord.RichEmbed()
            .setAuthor("𝕀ℂ𝔼 𝕊𝔼𝕃𝔽𝔹𝕆𝕋") 
            .setColor("#000000")
            .setTimestamp()
            .addField(`𝕊𝔼𝕋𝕌ℙ ℂ𝕆𝕄𝕄𝔸ℕ𝔻𝕊`, '`Stream` `Listen` `Watch` `Play` `Stop`')
            .setFooter(msg.author.tag, msg.author.displayAvatarURL)
        ...
        if (haste.toLowerCase() === "text") {
            ...
            .setTimestamp()
            .setFooter(msg.author.tag, msg.author.displayAvatarURL)
            .addField("**𝕋𝔼𝕏𝕋 ℂ𝕆𝕄𝕄𝔸ℕ𝔻𝕊**", "`Emojify`\n `Ascii`\n `Lol`\n `Zalgo`\n`Edgy`\n`Reverse`\n `Space`\n `Embed`")
            msg.channel.send(gay).then(message => message.delete(240000));
        } else
        ...
        if (haste.toLowerCase() === "nuke") {
            ...
            .setTimestamp()
            .setFooter(msg.author.tag, msg.author.displayAvatarURL)
            .addField("**𝕎𝕀ℤℤ ℂ𝕆𝕄𝕄𝔸ℕ𝔻𝕊**", "`Banall` - Bans all users \n `Kickall` - Kicks all users \n `Voiceraid` - Creates 100 voice channels \n `Textraid` - Creates 100 text channels \n `Deleteall` - Deletes all channels \n `lie` - Bans and deletes everything")
            msg.channel.send(gay).then(message => message.delete(240000));
        }
}
